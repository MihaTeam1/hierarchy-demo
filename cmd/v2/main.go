...
func (s Storage) ListAttributes(t []string) []HierarchyAttributes {
	...
	for _, attr := range s.attributes {
		if slices.Contains(t, attr.Type) {
...
func (p *PolicyManager) GetFirstManagers(empID string) ([]Employee, error) {
	...
	attrs := p.d.AttributeSource.ListAttributes([]string{"position", "employee"})

	var employeeDepartment string
	for _, attr := range attrs {
		if attr.Type == "employee" && attr.ID == emp.ID {
			var ok bool
			employeeDepartment, ok = GetAttribute[string](attr, "department")
			if !ok {
				return nil, fmt.Errorf("emp has no department")
			}
			break
		}
	}
	...
	var positionAttributes HierarchyAttributes
	for _, attr := range attrs {
		if attr.Type == "position" && attr.ID == position.ID {
			positionAttributes = attr
		}
	}

	managerPositionID, ok := GetAttribute[string](positionAttributes, "managerPosition")
	if !ok {
		return nil, nil // if there is no emp positions above it's ok
	}

	managers := p.d.EmployeeSource.ListEmployeesByPosition([]string{managerPositionID})

	var resp []Employee
	for _, attr := range attrs {
		if attr.Type != "employee" {
			continue
		}

		for _, manager := range managers {
			if manager.ID != attr.ID {
				continue
			}

			department, ok := GetAttribute[string](attr, "department")
			if !ok || department != employeeDepartment {
				continue
			}
